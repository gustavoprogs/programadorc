package br.com.programadorc.dao;

import br.com.programadorc.domain.Usuario;
import br.com.programadorc.util.HibernateUtil;
...
public class UsuarioDao {
    ...
    public Usuario consultar(Integer id) {
        ...
        session.beginTransaction();
        try {
            Usuario usuario = (Usuario) session.createQuery("from Usuario where nomero = " + id).uniqueResult();
            session.getTransaction().commit();
            return usuario;
        } catch (Exception e) {
            session.getTransaction().rollback();
